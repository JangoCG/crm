...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($_POST["mail"], FILTER_VALIDATE_EMAIL) === false) {
        ...
        $mailError = "Bitte geben Sie eine validierte Email-Adresse ein";
    }
    //Straße
    ...
    if (empty($vornameError) && empty($nachnameError) && empty($firmaError) && empty($mailError) && empty($mailError) && empty($plzError) && empty($landError) && empty($strasseError) && empty($hausnummerError) && empty($stadtError)) {
        //Überprüfung ob Mitarbeiter schon existiert
        $sqlPrüfen = "SELECT Vorname, Nachname, PLZ, Strasse, Hausnummer FROM mitarbeiter 
        ...
        if(mysqli_num_rows($resultPrüfen) > 0){
            $datenbankError = "Bitte legen Sie einen noch nicht vorhandenen Account an";
        }
        else {
            //SQL Statement Variable übergeben
            $sqlStatement = "INSERT INTO mitarbeiter (Vorname, Nachname,Firma, Email,  PLZ, Land, Strasse, Hausnummer, Stadt) 
                     VALUES ('$vorname','$nachname','$firma','$mail', '$plz', '$land', '$strasse', '$hausnummer', '$stadt')";
        ...
        <div class="col">
            ...
            <div class="one-container">
                ...
                <div>
                    ...
                    <input type="text" name="hausnummer" value="<?php echo $hausnummer; ?>" class="ml-2 kleines-feld">
