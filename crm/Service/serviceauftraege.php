...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($auftraggeberError) && empty($ansprechpartnerError) && empty($statusError) && empty($beschreibungError) && empty($garantieError) && empty($alterError) && empty($produktError)) {
        //Überprüfen ob der Serviceauftrag schon in der Datenbank vorhanden
        $sqlPrüfen = "SELECT Auftraggeber, Alter, Beschreibung FROM serviceauftrag
        ...
        if (mysqli_num_rows($resultPrüfen) >= 1) {
            ...
            $sqlStatement = "INSERT INTO serviceauftrag (Auftraggeber, Ansprechpartner, Beschreibung, Status, Garantie, Produkt, Alter) 
            ...
            if (mysqli_query($connection, $sqlStatement)) {
                echo "Serviceauftrag erfolgreich angelegt";
            } else {
...
<head>
    <title>Serviceauftrag anlegen</title>
    <?php
...
<div id="wrapper">
    ...
    ?>
    <div class="row">
        <form action="serviceauftraege.php" method="post">

            <div class="col">
                <h1>Serviceauftrag anlegen</h1>
            ...
            <div class="col">
                <h1>&nbsp;</h1>
                <div class="one"><h4>Verarbeitungsdaten</h4></div>
                <div class="one-container">
                    <div>
                        <label style="width:109.6px;">Status</label>
                        <select id="rolle" name="status[]" class="ml-2">
                            <option>offen</option>
                    ...
                    <div>
                        ...
                        <button class="btn btn-primary such-button" type="submit">Serviceauftrag anlegen</button>
